...
$username = $($wvdoptimizationtoolconfig.wvdoptimizationtoolconfig.domainName + "\" + $wvdoptimizationtoolconfig.wvdoptimizationtoolconfig.domainJoinUsername)
...
$scriptBlock = { .\psexec /accepteula -h -u $username -p $domainJoinPassword -c -f "powershell.exe" net localgroup 'FSLogix Profile Exclude List' $username /add }
Invoke-Command $scriptBlock -Verbose
